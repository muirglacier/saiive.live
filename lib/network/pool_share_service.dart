...
abstract class IPoolShareService {
  Future<List<PoolShare>> getMyPoolShare(String coin, List<String> addresses);
...
class PoolShareService extends NetworkService implements IPoolShareService {
  Future<List<PoolShare>> getMyPoolShare(String coin, List<String> addresses) async {
    ...
    dynamic response = await this.httpService.makeHttpPostRequest('/listminepoolshares', coin, request);

    if (response is ErrorResponse) {
      this.handleError(response);
    }

    List<PoolShare> poolShares = json.decode(response.body).values.map<PoolShare>((data) => PoolShare.fromJson(data)).toList();

    return poolShares;
  }

  Future<List<PoolShare>> getPoolShares(String coin) async {
    dynamic response = await this.httpService.makeHttpGetRequest('/listpoolshares/0/100000/true', coin);

    if (response is ErrorResponse) {
      this.handleError(response);
    }

    List<PoolShare> poolShares = response.values.map<PoolShare>((data) => PoolShare.fromJson(data)).toList();
