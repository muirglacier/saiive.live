...
class _VaultAddCollateral extends State<VaultAddCollateral> {
  ...
  Widget _buildAddCollateralPanel() {
    return Navigated(
        child: VaultAddCollateralTokenScreen(
            ...
            this.changes,
            (token, amount) => this.handleChangeAddCollateral(token, amount)
        )
    );
  }
  ...
  Widget _buildChangeCollateralPanel(LoanVaultAmount amount) {
    ...
    return Navigated(
        child: VaultEditCollateralTokenScreen(
          amount, balance,
            (loanAmount, amount) =>
                this.handleChangeEditCollateral(loanAmount, amount)));
  }
  ...
  handleRemoveCollateral(LoanVaultAmount loanAmount) {
    var existing = this.changes.keys.firstWhere(
        (element) => element == loanAmount.symbolKey,
        orElse: () => null);
    var existingCollateral = _collateralAmounts.firstWhere(
        (element) => element.symbolKey == loanAmount.symbolKey,
        orElse: () => null);
